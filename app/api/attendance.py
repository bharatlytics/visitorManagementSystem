...
def create_attendance():
    ...
    try:
        ...
        attendance_doc = {
            ...
            'attendanceTime': (lambda ts: datetime.fromisoformat(ts.replace('Z', '+00:00')).replace(tzinfo=None) if ts else now)(data.get('attendanceTime')),
    ...
    try:
        ...
        for record in records:
            ...
            attendance_doc = {
                ...
                'employeeId': ObjectId(employee_id) if employee_id else None,
                'visitorId': ObjectId(visitor_id) if visitor_id else None,
                'attendanceTime': (lambda ts: datetime.fromisoformat(ts.replace('Z', '+00:00')).replace(tzinfo=None) if ts else now)(record.get('attendanceTime')),
